Detectizr.detect({detectScreen:false});
 ...
 var off = angular.module('off',['serve_http']).run(['$rootScope','$location','$http', function($rootScope, $location, $http){
	$http.post('//'+$location.$$host+'/index.php/bundle/index/getapi', {cache: true}).then(function(resp){
		...
		var manual_price_set = function(discprice, text){

			jQuery("#bundle-offer-prices").find('.price').css('text-decoration', 'line-through');

			var $chkd_ele = jQuery("input[name='chosen']:checked");

			var discounted_price = discprice?discprice:$chkd_ele.siblings('.price-sec').children('span').text();

			jQuery(".bundle-offer").val(text?text:$chkd_ele.siblings('.name-wrap').children('.details-wrap').text());

			if (jQuery(".dis_counted_price").length > 0) {
			    jQuery(".dis_counted_price").text(discounted_price);
			} else {
			    jQuery("#bundle-offer-prices").after("<span class='dis_counted_price'>" + discounted_price + "<span>");
			}
		};

		manual_price_set();

			jQuery("#bundle-cat-list .off_holder").mCustomScrollbar({
				 theme:"dark",
				 scrollButtons:{ enable: true }
			});

			jQuery("#bundle-cat-list").on('click', 'a', function(e){
				e.preventDefault();
				//console.log(jQuery(this), jQuery(this).data())
				var $data = jQuery(this).data();
				manual_price_set($data.discounted_price, jQuery(this).text());
				$http.post('//'+$location.$$host+'/index.php/bundle/index/setsessiondata', {cache: true, 'cat_name':$data.cat_name, 'bundle_id':$data.bundle_id, 'discount':$data.discount}).then(function(){
		...
		jQuery("input[name='chosen']").change(function(){
			//console.log($("#product_addtocart_form"));
			//jQuery("#product_addtocart_form").attr('action','').submit();
			$chosen = jQuery("input[name='chosen']:checked")
			//console.log(jQuery("input[name='chosen']:checked"), jQuery("input[name='chosen']:checked").data());		
			manual_price_set();
			$http.post('//'+$location.$$host+'/index.php/bundle/index/setsessiondata', {cache: true, 'cat_name':$chosen.data('cat_name'), 'bundle_id':$chosen.data('bundle_id'), 'discount':$chosen.val()})
			//serve_http.get_details('http://magentoshop.staging-websites.com/', {cache: true, 'cat_name' : $rootScope.client_id, 'bundle_id' : $rootScope.client_secret, 'discount' : 'client_credentials'})
		})
			...
			return{
				template:
						...
						"<span class='bundle_reload' ng-if='show_loading'></span>"+
							"<div ng-cloak><span class='bndl_scrl_up' ng-if='category_size > 2' ng-click='push_up($event)'></span><ul class='bndl_ul'>"+
								"<li ng-repeat='(catName, catDetails) in categories | orderBy:catName' ng-if='catName !=\"have_more_offers\"' ng-init='$last ? initialize_scroll():null'>"+
									...
									"<div ng-repeat='detail in catDetails track by $index' ng-if='$index >1' class='bndl_catdet'>"+
										"<div ng-repeat='det in detail track by $index' class='bndl_off'>"+
											"<a ng-click='redirect_with_offer(det.bundle_object_id, det.product_category, det.discount, actualprice, $event)'>"+
								...
								"</li>"+
							"</ul><span class='bndl_scrl_dn' ng-if='category_size > 2' ng-click='push_dn($event)'></span>"+
				...
				controller:function($scope, $rootScope, $compile, serve_http){
					...
					$scope.show_bundle_price_category = $rootScope.show_bundle_price_product;
					//$scope.action = $rootScope.action;
					$scope.pager = 2;
					
					$scope.$on('url_given', function(s, u){
						console.log('url_given', u);
					})
					...
					$scope.redirect_with_offer = function(bid, bcname, discount, actualprice, ev){
						ev.stopPropagation();
					...
					$scope.$on('offerTempUpdated', function(s, ds){
						...
						$rootScope.currency = ds.currency;
		...
		off.directive('url',[function(){
			return{
				...
				link:function($scope, elem, attrs){
					...
					$scope.emit('url_given', attrs.href);
