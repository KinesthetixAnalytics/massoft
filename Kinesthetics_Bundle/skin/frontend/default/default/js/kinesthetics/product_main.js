jQuery(function(){
if(jQuery("#product-options-wrapper > dl").children().length == 2 && jQuery("#product-options-wrapper > dl").children('.have-bundle-option').length){
	jQuery("#product-options-wrapper").css({'padding':0, 'border-bottom':0});
...
Detectizr.detect({detectScreen:false});
 ...
 var off = angular.module('off',['serve_http', 'ngSanitize']).run(['$rootScope','$location','$http', function($rootScope, $location, $http){
...
off.directive('bundleMainWraper', ['$compile','$window', 'serve_http','$rootScope', function($compile, $window, serve_http, $rootScope){
	return{
		restrict:'C',
		scope:true,
		link:function($scope, elem, attrs){
			!jQuery.trim($rootScope.catname).length?jQuery(elem).hide():null;
			var $crss = $compile('<span class="bndl_cross" ng-click="unsetnredirect($event)" title="Click here to opt out of the bundle option"></span>')($scope);
			jQuery(elem).prepend($crss);
			$scope.unsetnredirect = function(ev){
				serve_http.get_details('http://magentoshop.staging-websites.com/index.php/bundle/index/unsetsessiondata').then(function(){
...
off.directive('tnc', ['$rootScope', '$sce', function($rootScope){
	return{
		template:"<div class='terms_n_condition' ng-if='tnc'><h2 ng-init='tnc_initialize()'><span class='signatured_image'></span> Terms of Bundle Offer</h2><div ng-bind-html='tnc' class='tncdetails'></div></div>",
		restrict:'A',
		...
		link:function($scope, elem, attrs){
			$scope.$on('off_slctd', function(s, t){
				$scope.tnc = t.terms_condition;
			});

			$scope.tnc_initialize = function(){
				
					
					jQuery(elem).find('.tncdetails').hide();
					jQuery(elem).find('h2').addClass('closed');
					if(!jQuery(elem).data('tncapplied')){
						jQuery(elem).data('tncapplied', true);
						jQuery(elem).on('click', 'h2', function(){
							jQuery(this).toggleClass('opened').next().stop(true, true).slideToggle();
						})
					}
			}
...
off.directive('usedOffer', ['$rootScope', 'serve_http', '$window', function($rootScope, serve_http, $window){
	return{
		template:"<div class='bundle-offers bundle-offer-block' id='usedOffer' ng-show='appliedbundle' ng-cloak><h2><span class='signatured_image'></span> Bundle Offers</h2>"+
				"<h4><img src='{{bndlimg}}'/><span>{{cataname?cataname:catname}}</span></h4><ul id='offered_slct'>"+
				"<li ng-repeat='offer in offers'><a href='' class='close_bundle_off' title='Click here to opt out of the bundle option' ng-click='deselect_offer(offer, $index, $event)'>close</a><h5 class='bndl_cat_name'>{{offer.bundle_object_name}}</h5>"+
					"<input type='radio' name='uoff_chosen' ng-init='checkme($index, offer.bundle_object_id, offer)?callTnC(offer):null' ng-checked='checkme($index, offer.bundle_object_id, offer)' ng-click='reassignOffer($event, offer)'>"+
					...
					"<span class='name-wrap'>"+
	                    ...
	                    "<div class='details-wrap'>"+
	                        ...
	                        "<label>from {{offer.retailer_URL}}</label>"+
	                    "</div>"+
		...
		link:function($scope, elem, attrs){
			...
			$scope.deselect_offer = function(o,i,ev){
				...
				serve_http.get_details('http://magentoshop.staging-websites.com/index.php/bundle/index/unsetsessiondata').then(function(){
					...
					if($scope.offers.length==0 || jQuery(ev.target).siblings('input').is(":checked") == true){
			...
			$scope.checkme = function(index, boi, offer){
			...
			$scope.callTnC = function(offer){
					console.log(offer.bundle_URL);
					jQuery("#bundle_url").val(offer.bundle_URL);
					$rootScope.$broadcast('off_slctd', offer);
			}
			...
			$scope.$on('usedOffer', function(s, o, cnm, i, discount, bndlid, bndlimg, d){
...
off.directive('bundleOfferPrices',[ function(){
	return{
		...
		link:function($scope, elem, attrs, $compile){
			...
			$scope.$on('offer_changed', function(s,e,f,g,h, i, j, k){
				//console.log(s, e, f, g, h, i, j, k);
...
off.directive('restCat', ['serve_http', function(serve_http){
	return{//ng-click='change_cat_off($event, catname, catdet.details[0].category_id)'
		template:"<div class='res_cat_holder' ng-show='appliedbundle'><span class='signatured_image'></span><h4>Choose a different category to bundle with:</h4><ul><li ng-repeat='(catname, catdet) in offers track by $index' ng-if='(catname != catnm) && $index'><a href='' ><img ng-src='{{catdet.image}}' width='50' alt='{{catname}}'><h4><span>Change bundle product to </span>{{catname}}</h4></a>"+
					"<ul>"+
						"<li ng-repeat='det in catdet.details track by $index'>"+
							"<a id='det.category_id' ng-click='redirect_with_offer(det.bundle_object_id, det.product_category, det.category_id, $index, det.discount, catdet.image, det, $event)'>"+
		...
		controller:function($scope, $rootScope){
			...
			$scope.redirect_with_offer = function(bndlid, catnm, catid, index, discount, bndlimg, det, ev){
				//console.log('hhhhhhhhhi');

				$rootScope.$broadcast('get_all_offers', catnm, catid, index, discount, bndlid, bndlimg, det);
			};
			
			$scope.$on('get_all_offers', function(s, cnm, catid, i, discount, bndlid, bndlimg, d){
				$scope.offers = $rootScope.all_relevent_offers;
				$scope.catnm = cnm;

				if(catid){
					$scope.change_cat_off(null, cnm, catid, i, discount, bndlid, bndlimg, d);
				}
			});

			var catn;
			$scope.change_cat_off = function(e, catname, catid, i, discount, bndlid, bndlimg, det){
				    ...
				    .then(function(rep) {			    	
				            serve_http.get_details('http://bundledserver.staging-websites.com/api_1_0_0/user/getProductDetailsAccordingProductCategory/', {
				                ...
				                .then(function(out) {
				                	...
				                	$rootScope.$broadcast('usedOffer', $scope.applied_offers_category, catname, i, discount, bndlid, bndlimg, det);
