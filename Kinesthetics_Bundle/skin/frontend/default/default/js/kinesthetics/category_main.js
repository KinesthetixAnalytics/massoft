...
var off = angular.module('off', ['serve_http']).run(['$rootScope', '$location', '$http', function($rootScope, $location, $http) {
    $http.post('//' + $location.$$host + '/index.php/bundle/index/getapi', {
        cache: true
    }).then(function(resp) {
        ...
        $rootScope.bundle_url = 'http://bundledserver.staging-websites.com/api_1_0_0/user/';
        $rootScope.session = 'http://magentoshop.staging-websites.com/index.php/bundle/index/setsessiondata';
...
off.controller('off_controller', ['$scope', '$window', 'serve_http', '$rootScope', function($scope, $window, serve_http, $rootScope) {
    ...
    $scope.$on('event_singularity', function(s, o) {
        ...
        timt = setTimeout(function() {
            $scope.catId = o.cId;
           $rootScope.bundle_url &&
            serve_http.get_details($rootScope.bundle_url+'token/', {
                    cache: true,
                ...
                .then(function(rep) {
                    serve_http.get_details($rootScope.bundle_url + 'getProductDetailsAccordingProductCategoryGroupByProductId/', {
...
off.directive('offerd', ['$rootScope', 'serve_http', '$window', function($rootScope, serve_http, $window) {
    return {
        template: "<form action='{{::action}}' class='bun_let' method='post'><input type='hidden' name='bundle_ob_id'></input><input type='hidden' name='bundle_cat_name'></input>" +
            ...
            "<a ng-click='redirect_with_offer(det, $event)'>" +
        ...
        controller: function($scope, $rootScope, $compile, serve_http) {
            ...
            $scope.pager = $rootScope.no_of_categories;

            $scope.reassignOffer = function(offer, ev) {
                $scope.selected_off = offer.bundle_object_id | $scope.selected_off;
                serve_http.get_details($rootScope.session, {
                        cat_name: offer.catname,
                        bundle_id: $scope.selected_off,
                        discount: offer.discount ? offer.discount : offer
                    })
                    .then(function() {
            ...
            $scope.redirect_with_offer = function(det, ev) {
                var z = {};
                z.bundle_object_id = det.bundle_object_id;
                z.catname = det.product_category;
                z.discount = det.discount;
                $scope.reassignOffer(z, ev);
            };
        },
        ...
        link: function($scope, elem, attrs, $compile) {
            ...
            $scope.show_more_categories = function(ev) {
                ...
                serve_http.get_details($rootScope.bundle_url+'token/', {
                    ...
                    .then(function(rep) {
                        serve_http.get_details($rootScope.bundle_url + 'getProductDetailsAccordingProductCategory/', {
    ...
    return {
        ...
        controller: function($scope, $http, $location, $rootScope, $timeout, $window) {
            ...
            $scope.fix_pos = function($telem) {
                ...
                if ($sct.scrollTop() + 20 >= $telem.children('div').offset().top) {
                    $telem.css({
                        'top': (parseInt($telem.css('top')) ? parseInt($telem.css('top')) : 0) + 40 + 'px',
                        bottom: 'auto'
                    });
                } else {
                    ...
                    if ($telem.children('div').offset().top + $telem.children('div').height() + 40 >= $sct.scrollTop() + $scope.win_height) {
                        $telem.css({
                            'bottom': (parseInt($telem.css('bottom')) ? parseInt($telem.css('bottom')) : 0) + 20 + 'px',
                            top: 'auto'
                        })
                    }
                ...
                if ($telem.children('div').offset().left + $telem.children('div').width() + 20 >= $scope.win_width) {
                    if ($telem.children('div').width() + 40 > $scope.win_width) {
                        ...
                        $telem.css({
                            'right': (parseInt($telem.css('right')) ? parseInt($telem.css('right')) : 0) + 20 + 'px',
                            left: 'auto',
                            'visibility': 'visible'
                        });
                    }
        ...
        link: function($scope, elem, attrs, $location, $http) {
            ...
            $scope.$on('rootscope_updated', function() {
                if (parseInt($parentel.offset().top) <= jQuery(window).innerHeight() * 4) {
                    jQuery(elem).data('done_ajax', true);
                    $scope.$emit('event_singularity', {
                        pId: $scope.prodId,
                        cId: $scope.catId
                    });
                }
...
serve_http.factory('serve_http', function($http) {
    function get_details(u, p) {
        return $http.post(u, (p ? p : null))
    };
    return {
